...
func (g *generator) renderRuntimeHelpers(buf *bytes.Buffer) {
	...
	fmt.Fprintf(buf, "\t\treturn nil, fmt.Errorf(\"Array index out of bounds\")\n")
	fmt.Fprintf(buf, "\t}\n")
	...
	fmt.Fprintf(buf, "\tif arr, ok := base.(*runtime.ArrayValue); ok && arr != nil && arr.Handle == 0 {\n")
	...
	fmt.Fprintf(buf, "\t}\n")
	fmt.Fprintf(buf, "\tif arr != nil && arr.Handle == 0 {\n")
	...
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_raise_overflow(node ast.Node) {\n")
	fmt.Fprintf(buf, "\tif __able_runtime == nil {\n")
	fmt.Fprintf(buf, "\t\tpanic(fmt.Errorf(\"compiler: missing runtime\"))\n")
	fmt.Fprintf(buf, "\t}\n")
	fmt.Fprintf(buf, "\tbridge.RaiseWithContext(__able_runtime, node, bridge.OverflowError(__able_runtime, \"integer overflow\"))\n")
	fmt.Fprintf(buf, "}\n\n")
	...
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_raise_runtime_error(node ast.Node, message string) {\n")
	fmt.Fprintf(buf, "\tif __able_runtime == nil {\n")
	fmt.Fprintf(buf, "\t\tpanic(fmt.Errorf(\"compiler: missing runtime\"))\n")
	fmt.Fprintf(buf, "\t}\n")
	...
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_checked_add_signed(a int64, b int64, width int, node ast.Node) int64 {\n")
	...
	fmt.Fprintf(buf, "\t\t\t__able_raise_overflow(node)\n")
	...
	fmt.Fprintf(buf, "\treturn result\n")
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_checked_sub_signed(a int64, b int64, width int, node ast.Node) int64 {\n")
	fmt.Fprintf(buf, "\tif width >= 64 {\n")
	fmt.Fprintf(buf, "\t\tresult := int64(uint64(a) - uint64(b))\n")
	fmt.Fprintf(buf, "\t\tif (a >= 0 && b < 0 && result < 0) || (a < 0 && b > 0 && result >= 0) {\n")
	fmt.Fprintf(buf, "\t\t\t__able_raise_overflow(node)\n")
	...
	fmt.Fprintf(buf, "\treturn result\n")
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_checked_mul_signed(a int64, b int64, width int, node ast.Node) int64 {\n")
	fmt.Fprintf(buf, "\tif width >= 64 {\n")
	...
	fmt.Fprintf(buf, "\t\t\t\treturn 0\n")
	fmt.Fprintf(buf, "\t\t\t}\n")
	fmt.Fprintf(buf, "\t\t\treturn int64(lower)\n")
	fmt.Fprintf(buf, "\t\t}\n")
	...
	fmt.Fprintf(buf, "\treturn result\n")
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_checked_add_unsigned(a uint64, b uint64, width int, node ast.Node) uint64 {\n")
	fmt.Fprintf(buf, "\tif width >= 64 {\n")
	...
	fmt.Fprintf(buf, "\treturn result\n")
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_checked_sub_unsigned(a uint64, b uint64, width int, node ast.Node) uint64 {\n")
	...
	fmt.Fprintf(buf, "\treturn result\n")
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_checked_mul_unsigned(a uint64, b uint64, width int, node ast.Node) uint64 {\n")
	fmt.Fprintf(buf, "\tif width >= 64 {\n")
	...
	fmt.Fprintf(buf, "\treturn result\n")
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_pow_signed(base int64, exp int64, bits int, node ast.Node) int64 {\n")
	fmt.Fprintf(buf, "\tif exp < 0 {\n")
	fmt.Fprintf(buf, "\t\t__able_raise_runtime_error(node, \"Negative integer exponent is not supported\")\n")
	fmt.Fprintf(buf, "\t\treturn 0\n")
	fmt.Fprintf(buf, "\t}\n")
	...
	fmt.Fprintf(buf, "\t\t\tresult = __able_checked_mul_signed(result, current, bits, node)\n")
	fmt.Fprintf(buf, "\t\t}\n")
	fmt.Fprintf(buf, "\t\tvalue >>= 1\n")
	fmt.Fprintf(buf, "\t\tif value > 0 {\n")
	fmt.Fprintf(buf, "\t\t\tcurrent = __able_checked_mul_signed(current, current, bits, node)\n")
	fmt.Fprintf(buf, "\t\t}\n")
	fmt.Fprintf(buf, "\t}\n")
	fmt.Fprintf(buf, "\treturn result\n")
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_pow_unsigned(base uint64, exp uint64, bits int, node ast.Node) uint64 {\n")
	fmt.Fprintf(buf, "\tresult := uint64(1)\n")
	fmt.Fprintf(buf, "\tcurrent := base\n")
	fmt.Fprintf(buf, "\tvalue := exp\n")
	fmt.Fprintf(buf, "\tfor value > 0 {\n")
	fmt.Fprintf(buf, "\t\tif value&1 == 1 {\n")
	fmt.Fprintf(buf, "\t\t\tresult = __able_checked_mul_unsigned(result, current, bits, node)\n")
	fmt.Fprintf(buf, "\t\t}\n")
	fmt.Fprintf(buf, "\t\tvalue >>= 1\n")
	fmt.Fprintf(buf, "\t\tif value > 0 {\n")
	fmt.Fprintf(buf, "\t\t\tcurrent = __able_checked_mul_unsigned(current, current, bits, node)\n")
	fmt.Fprintf(buf, "\t\t}\n")
	fmt.Fprintf(buf, "\t}\n")
	fmt.Fprintf(buf, "\treturn result\n")
	fmt.Fprintf(buf, "}\n\n")
	...
	fmt.Fprintf(buf, "\treturn val\n")
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_divmod_signed(a int64, b int64, bits int, node ast.Node) (int64, int64) {\n")
	...
	fmt.Fprintf(buf, "}\n\n")
	fmt.Fprintf(buf, "func __able_divmod_unsigned(a uint64, b uint64, bits int, node ast.Node) (uint64, uint64) {\n")
	fmt.Fprintf(buf, "\tif b == 0 {\n")
	fmt.Fprintf(buf, "\t\t__able_raise_division_by_zero(node)\n")
	fmt.Fprintf(buf, "\t\treturn 0, 0\n")
	fmt.Fprintf(buf, "\t}\n")
