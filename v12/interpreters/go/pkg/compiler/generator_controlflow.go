...
func linesReferenceLabel(lines []string, label string) bool {
	...
	for _, line := range lines {
		if lineReferencesLabel(line, "break", label) || lineReferencesLabel(line, "continue", label) {
			return true
		}
	}
	return false
...
func lineReferencesLabel(line string, keyword string, label string) bool {
	if label == "" {
		return false
	}
	reference := keyword + " " + label
	searchFrom := 0
	for {
		offset := strings.Index(line[searchFrom:], reference)
		if offset < 0 {
			return false
		}
		offset += searchFrom
		end := offset + len(reference)
		if end == len(line) || !isGeneratedGoIdentifierByte(line[end]) {
			return true
		}
		searchFrom = end
	}
...
func isGeneratedGoIdentifierByte(ch byte) bool {
	return ch == '_' ||
		(ch >= 'a' && ch <= 'z') ||
		(ch >= 'A' && ch <= 'Z') ||
		(ch >= '0' && ch <= '9')
...
func (g *generator) compileTailExpression(ctx *compileContext, expected string, expr ast.Expression) ([]string, string, string, bool) {
	...
	if expected != "" && (g.isVoidType(expected) || g.isNilType(expected)) {
		stmtLines, ok := g.compileStatement(ctx, expr)
		if !ok {
			return nil, "", "", false
		}
		if g.isNilType(expected) {
			return stmtLines, "runtime.NilValue{}", "runtime.NilValue", true
		}
...
func (g *generator) compileForLoopInternal(ctx *compileContext, loop *ast.ForLoop, withResult bool) ([]string, string, bool) {
	...
	lines = append(lines, "}")
	lines = append(lines, fmt.Sprintf("if !%s { %s.Close() }", isArrayTemp, iteratorTemp))
