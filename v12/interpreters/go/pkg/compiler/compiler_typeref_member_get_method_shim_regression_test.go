...
func TestCompilerRemovesTypeRefPointerMemberGetMethodShim(t *testing.T) {
	...
	if strings.Contains(segment, "__able_lookup_compiled_method(typed.TypeName, name, false)") {
		t.Fatalf("expected direct TypeRef member_get_method lookup shim call to be removed")
	}
	if strings.Contains(segment, "case runtime.TypeRefValue:") {
		t.Fatalf("expected dedicated TypeRef member_get_method static lookup shim branch to be removed")
	}
