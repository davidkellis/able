...
func (e *irGoEmitter) emitIteratorLiteral(node *IRIteratorLiteral) error {
	...
	fmt.Fprintf(&e.buf, "\t%s := func(gen runtime.Value) error {\n", runVar)
...
func (e *irGoEmitter) emitIteratorHelpers() {
	...
	fmt.Fprintf(&e.buf, "\tdone bool\n")
	fmt.Fprintf(&e.buf, "\tcontrol runtime.Value\n")
	fmt.Fprintf(&e.buf, "}\n\n")
	fmt.Fprintf(&e.buf, "func __able_new_iterator(rt *bridge.Runtime, run func(gen runtime.Value) error) *runtime.IteratorValue {\n")
	...
	fmt.Fprintf(&e.buf, "}\n\n")
	fmt.Fprintf(&e.buf, "func (g *__able_generator) run(run func(gen runtime.Value) error) {\n")
	...
	fmt.Fprintf(&e.buf, "\t\t\t}\n")
	fmt.Fprintf(&e.buf, "\t\t}()\n")
	fmt.Fprintf(&e.buf, "\t\trunErr = run(g.controllerValue())\n")
	fmt.Fprintf(&e.buf, "\t}()\n")
	...
	fmt.Fprintf(&e.buf, "}\n\n")
	fmt.Fprintf(&e.buf, "func (g *__able_generator) controllerValue() runtime.Value {\n")
	fmt.Fprintf(&e.buf, "\tg.mu.Lock()\n")
	fmt.Fprintf(&e.buf, "\tdefer g.mu.Unlock()\n")
	fmt.Fprintf(&e.buf, "\tif g.control != nil {\n")
	fmt.Fprintf(&e.buf, "\t\treturn g.control\n")
	fmt.Fprintf(&e.buf, "\t}\n")
	fmt.Fprintf(&e.buf, "\tyieldFn := runtime.NativeFunctionValue{\n")
	fmt.Fprintf(&e.buf, "\t\tName: \"__iterator_controller_yield\",\n")
	fmt.Fprintf(&e.buf, "\t\tArity: 1,\n")
	fmt.Fprintf(&e.buf, "\t\tImpl: func(_ *runtime.NativeCallContext, args []runtime.Value) (runtime.Value, error) {\n")
	fmt.Fprintf(&e.buf, "\t\t\tvar value runtime.Value = runtime.NilValue{}\n")
	fmt.Fprintf(&e.buf, "\t\t\tif len(args) > 0 {\n")
	fmt.Fprintf(&e.buf, "\t\t\t\tif len(args) > 1 {\n")
	fmt.Fprintf(&e.buf, "\t\t\t\t\treturn nil, fmt.Errorf(\"gen.yield expects at most one argument\")\n")
	fmt.Fprintf(&e.buf, "\t\t\t\t}\n")
	fmt.Fprintf(&e.buf, "\t\t\t\tvalue = args[0]\n")
	fmt.Fprintf(&e.buf, "\t\t\t}\n")
	fmt.Fprintf(&e.buf, "\t\t\tif err := g.emit(value); err != nil {\n")
	fmt.Fprintf(&e.buf, "\t\t\t\treturn nil, err\n")
	fmt.Fprintf(&e.buf, "\t\t\t}\n")
	fmt.Fprintf(&e.buf, "\t\t\treturn runtime.NilValue{}, nil\n")
	fmt.Fprintf(&e.buf, "\t\t},\n")
	fmt.Fprintf(&e.buf, "\t}\n")
	fmt.Fprintf(&e.buf, "\tcloseFn := runtime.NativeFunctionValue{\n")
	fmt.Fprintf(&e.buf, "\t\tName: \"__iterator_controller_close\",\n")
	fmt.Fprintf(&e.buf, "\t\tArity: 0,\n")
	fmt.Fprintf(&e.buf, "\t\tImpl: func(_ *runtime.NativeCallContext, _ []runtime.Value) (runtime.Value, error) {\n")
	fmt.Fprintf(&e.buf, "\t\t\tg.close()\n")
	fmt.Fprintf(&e.buf, "\t\t\treturn runtime.NilValue{}, nil\n")
	fmt.Fprintf(&e.buf, "\t\t},\n")
	fmt.Fprintf(&e.buf, "\t}\n")
	fmt.Fprintf(&e.buf, "\tstopFn := runtime.NativeFunctionValue{\n")
	fmt.Fprintf(&e.buf, "\t\tName: \"__iterator_controller_stop\",\n")
	fmt.Fprintf(&e.buf, "\t\tArity: 0,\n")
	fmt.Fprintf(&e.buf, "\t\tImpl: func(_ *runtime.NativeCallContext, _ []runtime.Value) (runtime.Value, error) {\n")
	fmt.Fprintf(&e.buf, "\t\t\tg.close()\n")
	fmt.Fprintf(&e.buf, "\t\t\treturn runtime.NilValue{}, __able_generator_stop{}\n")
	fmt.Fprintf(&e.buf, "\t\t},\n")
	fmt.Fprintf(&e.buf, "\t}\n")
	fmt.Fprintf(&e.buf, "\tg.control = &runtime.StructInstanceValue{\n")
	fmt.Fprintf(&e.buf, "\t\tFields: map[string]runtime.Value{\n")
	fmt.Fprintf(&e.buf, "\t\t\t\"yield\": yieldFn,\n")
	fmt.Fprintf(&e.buf, "\t\t\t\"close\": closeFn,\n")
	fmt.Fprintf(&e.buf, "\t\t\t\"stop\":  stopFn,\n")
	fmt.Fprintf(&e.buf, "\t\t},\n")
	fmt.Fprintf(&e.buf, "\t}\n")
	fmt.Fprintf(&e.buf, "\treturn g.control\n")
	fmt.Fprintf(&e.buf, "}\n\n")
