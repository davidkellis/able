...
func (g *generator) finishNativeCallableCall(ctx *compileContext, lines []string, callableExpr string, info *nativeCallableInfo, argExprs []string, callNode string, expected string) ([]string, string, string, bool) {
	if g == nil || ctx == nil || info == nil || callableExpr == "" {
		return nil, "", "", false
	}
...
func (g *generator) compileNativeCallableCall(ctx *compileContext, call *ast.FunctionCall, expected string, callableExpr string, callableType string, fnTypeExpr *ast.FunctionTypeExpression, callNode string) ([]string, string, string, bool) {
	...
	return g.finishNativeCallableCall(ctx, lines, callableExpr, info, args, callNode, expected)
}
