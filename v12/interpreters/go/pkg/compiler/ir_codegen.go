...
func (e *irGoEmitter) emitHelpers() {
	...
	fmt.Fprintf(&e.buf, "\t\tif value, ok := err.Payload[\"value\"]; ok {\n")
	fmt.Fprintf(&e.buf, "\t\t\tif inst, ok, nilPtr := __able_runtime_struct_instance_value(value); ok || nilPtr {\n")
	fmt.Fprintf(&e.buf, "\t\t\t\tif !ok || nilPtr {\n")
	fmt.Fprintf(&e.buf, "\t\t\t\t\treturn nil\n")
	fmt.Fprintf(&e.buf, "\t\t\t\t}\n")
	fmt.Fprintf(&e.buf, "\t\t\t\treturn inst\n")
	fmt.Fprintf(&e.buf, "\t\t\t}\n")
...
func (e *irGoEmitter) emitStructLiteral(node *IRStructLiteral) error {
	...
	if len(node.Updates) > 0 {
		...
		for _, update := range node.Updates {
			...
			fmt.Fprintf(&e.buf, "\tfor _, defField := range %s.Definition.Node.Fields { if defField == nil || defField.Name == nil { continue }; fieldValue, ok := __able_struct_named_field_value(%s, defField.Name.Name); if !ok { return nil, fmt.Errorf(\"functional update source missing field '%%s'\", defField.Name.Name) }; %s[defField.Name.Name] = fieldValue }\n", instTemp, instTemp, fieldsTemp)
		}
...
func (e *irGoEmitter) emitMapLiteral(node *IRMapLiteral) error {
	...
	for _, element := range node.Elements {
		...
		case IRMapSpread:
			...
			fmt.Fprintf(&e.buf, "\t\t}\n")
			fmt.Fprintf(&e.buf, "\t\thandle, ok := __able_struct_named_field_value(inst, \"handle\")\n")
			fmt.Fprintf(&e.buf, "\t\tif !ok {\n")
			fmt.Fprintf(&e.buf, "\t\t\tpanic(fmt.Errorf(\"map literal spread expects HashMap value\"))\n")
			fmt.Fprintf(&e.buf, "\t\t}\n")
