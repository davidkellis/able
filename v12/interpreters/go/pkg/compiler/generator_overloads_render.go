...
func (g *generator) renderMethodOverloadDispatchers(buf *bytes.Buffer) {
	...
	g.renderPublicPackageMethodDispatchers(buf)
}

func (g *generator) renderPublicPackageMethodDispatchers(buf *bytes.Buffer) {
	if g == nil || buf == nil {
		return
	}
	packages := g.registrationPackageList()
	for _, pkgName := range packages {
		...
		for _, group := range groups {
			...
			wrapperName := g.publicPackageMethodCallableWrapperName(pkgName, group.MethodName)
			displayName := fmt.Sprintf("%s.%s", pkgName, group.MethodName)
			fmt.Fprintf(buf, "func %s(rt *bridge.Runtime, ctx *runtime.NativeCallContext, args []runtime.Value) (runtime.Value, error) {\n", wrapperName)
			fmt.Fprintf(buf, "\tif rt == nil {\n")
			fmt.Fprintf(buf, "\t\treturn nil, fmt.Errorf(\"compiler: missing runtime\")\n")
			fmt.Fprintf(buf, "\t}\n")
