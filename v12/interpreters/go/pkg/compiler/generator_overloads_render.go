...
func (g *generator) renderOverloadDispatchers(buf *bytes.Buffer) {
	...
	names := make([]string, 0, len(g.overloads))
	for name := range g.overloads {
		names = append(names, name)
	}
	sort.Strings(names)
	for _, name := range names {
		overload := g.overloads[name]
		if overload == nil || len(overload.Entries) == 0 {
			continue
		}
		wrapperName := g.overloadWrapperName(name)
		callName := g.overloadCallName(name)
		valueName := g.overloadValueName(name)
