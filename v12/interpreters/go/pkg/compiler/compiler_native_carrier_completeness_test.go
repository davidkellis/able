...
func TestCompilerParameterizedUnionAliasLocalWithImportedShadowedInterfaceStaysNative(t *testing.T) {
	result := compileNoFallbackPackage(t, "demo", map[string]string{
		"main.able": strings.Join([]string{
			...
			"",
			"fn main() -> i32 {",
			"  local: Choice (RemoteReader i32) = if true { First {} } else { \"ok\" }",
			"  local match {",
	...
	if !strings.Contains(body, "var local __able_union_") {
		t.Fatalf("expected parameterized union alias local with imported shadowed interface actual to use a native union carrier:\n%s", body)
	}
	for _, fragment := range []string{
		"var local runtime.Value",
		"var local any",
		...
		if strings.Contains(body, fragment) {
			t.Fatalf("expected parameterized union alias local with imported shadowed interface actual to avoid %q:\n%s", fragment, body)
		}
	}
	if !strings.Contains(body, "reader.read()") {
		t.Fatalf("expected parameterized union alias local with imported shadowed interface actual to keep direct native interface dispatch:\n%s", body)
	}
	...
	for _, fragment := range []string{
		...
		if strings.Contains(compiledSrc, fragment) {
			t.Fatalf("expected parameterized union alias local with imported shadowed interface actual to avoid residual runtime.Value union members %q:\n%s", fragment, compiledSrc)
		}
	}
}

func TestCompilerParameterizedResultAliasLocalWithImportedShadowedCallableStaysNative(t *testing.T) {
	result := compileNoFallbackPackage(t, "demo", map[string]string{
		"main.able": strings.Join([]string{
			"package demo",
			"",
			...
			"",
			"fn main() -> i32 {",
			"  local: Outcome (() -> RemoteThing) = if true {",
	...
	if !strings.Contains(body, "var local __able_union_") {
		t.Fatalf("expected parameterized result alias local with imported shadowed callable actual to use a native union carrier:\n%s", body)
	}
	for _, fragment := range []string{
		"var local runtime.Value",
		"var local any",
		...
		if strings.Contains(body, fragment) {
			t.Fatalf("expected parameterized result alias local with imported shadowed callable actual to avoid %q:\n%s", fragment, body)
		}
	}
	if !strings.Contains(body, ".Remote") {
		t.Fatalf("expected parameterized result alias local with imported shadowed callable actual to keep direct native field access on the callable result:\n%s", body)
	}
	...
	for _, fragment := range []string{
		...
		if strings.Contains(compiledSrc, fragment) {
			t.Fatalf("expected parameterized result alias local with imported shadowed callable actual to avoid residual runtime.Value union members %q:\n%s", fragment, compiledSrc)
		}
	}
}
