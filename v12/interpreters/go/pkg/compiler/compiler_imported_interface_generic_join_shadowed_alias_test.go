...
func importedInterfaceGenericJoinShadowedAliasPackageFiles() map[string]string {
	return map[string]string{
		"main.able": strings.Join([]string{
			"package demo",
			"",
			"import demo.remote.{Echo, Tagger, Box, Mirror, Labeler, Badge, Reader::RemoteReader, Thing::RemoteThing, Choice3, Outcome3, First}",
			"",
			...
			"  second := tagger.tagged<Outcome3 (() -> RemoteThing)>(second_input)",
...
func TestCompilerImportedJoinedGenericInterfaceShadowedMultiMemberAliasCallsStayNative(t *testing.T) {
	result := compileNoFallbackPackage(t, "demo", importedInterfaceGenericJoinShadowedAliasPackageFiles())
	...
	for _, fragment := range []string{
		...
		if strings.Contains(mainBody, fragment) {
			t.Fatalf("expected imported joined generic-interface shadowed multi-member alias calls to avoid %q:\n%s", fragment, mainBody)
		}
	}
	for _, fragment := range []string{
		...
		if !strings.Contains(mainBody, fragment) {
			t.Fatalf("expected imported joined generic-interface shadowed multi-member alias calls to include %q:\n%s", fragment, mainBody)
		}
	}
	...
	for _, fragment := range []string{
		...
		if !strings.Contains(compiledSrc, fragment) {
			t.Fatalf("expected imported joined generic-interface shadowed multi-member alias helpers to include %q:\n%s", fragment, compiledSrc)
		}
	}
	for _, fragment := range []string{
		...
		if strings.Contains(compiledSrc, fragment) {
			t.Fatalf("expected imported joined generic-interface shadowed multi-member alias helpers to avoid %q:\n%s", fragment, compiledSrc)
		}
	}
}
