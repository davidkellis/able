...
func (i *Interpreter) hashMapFindEntryWithHash(hm *runtime.HashMapValue, hash uint64, key runtime.Value) (int, bool, error) {
	if candidates, indexed := hm.HashCandidates(hash); indexed {
		for _, idx := range candidates {
			equal, err := i.HashMapKeysEqual(hm.Entries[idx].Key, key)
			if err != nil {
				return -1, false, err
			}
			if equal {
...
func (i *Interpreter) hashMapInsertEntry(hm *runtime.HashMapValue, key runtime.Value, value runtime.Value) error {
	...
	hm.AppendEntry(runtime.HashMapEntry{Key: key, Value: value, Hash: hash})
	return nil
}
...
func (i *Interpreter) HashMapKeysEqual(a, b runtime.Value) (bool, error) {
	...
	if equal, ok := fastPrimitiveHashMapKeyEqual(receiver, other); ok {
		return equal, nil
	}
...
func (i *Interpreter) HashMapHashValue(val runtime.Value) (uint64, error) {
	...
	if hash, ok, err := fastPrimitiveHashMapHash(receiver); ok || err != nil {
		return hash, err
	}
...
func fastPrimitiveHashMapKeyEqual(left, right runtime.Value) (bool, bool) {
...
func fastPrimitiveHashMapHash(val runtime.Value) (uint64, bool, error) {
...
func (i *Interpreter) resolveInterfaceMethod(receiver runtime.Value, interfaceName string, methodName string) (runtime.Value, error) {
	if callable, found, err := i.resolvePrimitiveInterfaceMethodCallable(receiver, methodName, interfaceName); err != nil || found {
