...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `fixed_width_128` | wide-numeric | 7.988 / 0.384 | 60,446,436 | 31.80% | 20.78x | 1.47x | 14.17x | 9.325 |
| `distance_field` | float-numeric | 5.748 / 0.441 | 78,000,165 | 41.03% | 13.03x | 1.70x | 7.69x | 9.588 |
| `concurrent_event_routing` | concurrency | 3.032 / 0.031 | 21,367,576 | 43.14% | 96.98x | 1.76x | 55.15x | 2.573 |
| `word_frequency` | text-map | 1.418 / 0.020 | 17,591,889 | 44.32% | 71.65x | 1.80x | 39.89x | 2.020 |
| `array_slice_window` | array-iterator | 0.648 / 0.031 | 8,751,239 | 32.79% | 20.94x | 1.49x | 14.07x | 5.261 |
| `reverse_complement` | byte-text | 3.290 / 0.028 | 60,602,670 | 30.09% | 117.50x | 1.43x | 82.15x | 0.661 |

Across the six applications, 89,338,836 of 246,759,975 operations are modeled transport (36.20%). The largest equal-cost planning gain is 1.80x, while the smallest remaining target requirement is still 7.69x.

The equal-cost calculation is deliberately favorable to transport removal: it charges each removed load/constant/pop the same average cost as a semantic operation, makes removal free, and adds no translation, register, merge, materialization, or code-size cost. It is a planning scenario, not a timing claim. The target nanoseconds show why representation work alone cannot close the budget: the surviving calls, branches, stores, collection operations, errors, and concurrency semantics would need to average 0.661-9.588 ns each.

## Live opcode closure

All 143 live opcodes are classified exactly once. Only 6 are removable representation dispatches; 137 retain observable or required semantic work.
...
| `calls-callables-dynamic-dispatch` | required-semantic-work | 13 |
...
The current VM surface contains 132 production `bytecode_vm*.go` files / 32,602 lines and 116 test files / 35,986 lines. A complete parallel executor would therefore duplicate a large semantic authority unless it replaced the primary dispatcher incrementally; the prior cold-fallback executor never reached that point.
...
v12/bench_bytecode_architecture_budget \
  --json-out v12/docs/perf-baselines/2026-07-21-bytecode-architecture-target-budget.json \
  --markdown-out v12/docs/perf-baselines/2026-07-21-bytecode-architecture-target-budget.md
