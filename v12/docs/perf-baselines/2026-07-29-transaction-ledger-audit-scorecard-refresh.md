...
- Source measurements through: `2026-07-29T19:33:10.278417Z`
...
- Compiled: `7/64` selected rankable rows meet the 95%-of-Go target.
- Bytecode: `4/64` selected rankable rows meet both 95%-of-Python and 95%-of-Ruby targets.
- Canonical Able source fingerprints: `128` row fingerprints in JSON; `128` came from the measured source report and the remainder are current-source legacy fingerprints.
- Verifier/declared-input contracts: `128` row fingerprints in JSON; `128` were captured before the timed launch and the remainder are current-contract legacy reconstructions.
...
- Strict candidate selection: `128` reviewed benchmark/mode rows, SHA-256 `96d86cdad06f52286e4423ecad7d47ffaf75968fdc51a7e8010f65369291549e`; timeout rows remain in full status.
- Matched reference source fingerprints: `192` comparison fingerprints in JSON; `192` came from measured reference reports and the remainder are current-source legacy fingerprints.
