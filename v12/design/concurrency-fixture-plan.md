...
- Target fixtures: concurrency/future_cancel_value, future_memoization, future_cancelled_outside_error, future_cancelled_helper.
...
- 2025-10-24: Serial executor gained an explicit yield path so fixtures can assert deterministic round-robin traces. Added fairness fixtures (`concurrency/fairness_future_round_robin`, `concurrency/fairness_future_interleaving`) and wired them into both harnesses.

# Fairness Fixture Status

- **Serial executor coordination**: Implemented; `future_yield` now requeues serial tasks without touching production goroutine behaviour.
- **Fixture coverage**: Round-robin future fairness fixtures live under `fixtures/ast/concurrency/` and run green in TS + Go parity suites.
