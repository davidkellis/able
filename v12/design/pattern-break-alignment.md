# Break & Pattern Semantics Alignment (v11 Interpreters)

_Date: 2025-03-xx_

## Overview
Both Able v11 interpreters now share the same observable behaviour for:
...
- **Continue semantics**: Unlabeled `continue` simply skips the current loop iteration. Labeled `continue` is not part of Able v11; interpreters must reject it with the shared error message `"Labeled continue not supported"`.
...
- (Resolved) Spec clarified that labeled `continue` is not part of v11; no further action required beyond keeping error messaging aligned.
