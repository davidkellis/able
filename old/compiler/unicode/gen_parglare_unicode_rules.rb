...
def convert_min_ranges_to_lexer_rules(ranges)
  char_class = ranges.map do |range|
    if range.first == range.last
      # code_point = sprintf("%X", range.first).rjust(4, '0')
      # "\\u#{code_point}"
      format_code_point_for_python(range.first)
    else
      # code_point1 = sprintf("%X", range.first).rjust(4, '0')
      # code_point2 = sprintf("%X", range.last).rjust(4, '0')
      # "\\u#{code_point1}-\\u#{code_point2}"
      "#{format_code_point_for_python(range.first)}-#{format_code_point_for_python(range.last)}"
    end
  end.join()
  "/[#{char_class}]/"
end

def format_code_point_for_python(int_code_point)
  if int_code_point < 16**4
    code_point = sprintf("%X", int_code_point).rjust(4, '0')
    "\\u#{code_point}"
  else
    code_point = sprintf("%X", int_code_point).rjust(8, '0')
    "\\U#{code_point}"
  end
end
