...
importDecl
  ...
  | IMPORT LWS fqPackageIdentifier (LWS AS LWS packageIdentifier)? DOT STAR   # ImportDeclAllMembers
  | IMPORT LWS fqPackageIdentifier (LWS AS LWS packageIdentifier)? 
      DOT LBRACE (WS? identifier (LWS AS LWS identifier)? WS? COMMA)* 
                 (WS? identifier (LWS AS LWS identifier)?)? WS? RBRACE        # ImportDeclSpecificMembers
  ;
...
// v *= expr
assignmentExpr
  : multipleAssignmentLhs LWS? EQUAL LWS? expressionList                      # AssignmentExprMultipleAssignment
...
// Array A | bool | String
unionOfTypes: typeName (WS PIPE WS typeName)+ ;
...
expression
  ...
  | importDecl                                                          # ExpressionImportDecl
  | functionDefn                                                        # ExpressionFunctionDefn
...
fqPackageIdentifier: (packageIdentifier DOT)* packageIdentifier;

identifier: LETTER ( LETTER | UNICODE_DIGIT)* QUESTION?;
fqIdentifier: (identifier DOT)* identifier;   // fully qualified identifier
placeholderIdentifier: '_' DECIMAL_DIGIT* ;
...
FLOAT_LITERAL
  : DECIMAL_LITERAL DOT DECIMAL_LITERAL EXPONENT? FLOAT_WIDTH?
  | DECIMAL_LITERAL (EXPONENT | FLOAT_WIDTH | EXPONENT FLOAT_WIDTH)
	| DOT DECIMAL_LITERAL EXPONENT? FLOAT_WIDTH?
  ;

fragment EXPONENT
  : ( 'e' | 'E' ) ( PLUS | MINUS )? DECIMAL_LITERAL
  ;
