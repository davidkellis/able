...
describe("v10 interpreter - destructuring assignment", () => {
  ...
  test("destructuring assignment with = declares bindings when missing", () => {
    ...
    I.evaluate(AST.assignmentExpression("=", pat as any, arr));
    expect(I.evaluate(AST.identifier("left"))).toEqual({ kind: "i32", value: 4 });
    expect(I.evaluate(AST.identifier("right"))).toEqual({ kind: "i32", value: 5 });
  });
  ...
  test(":= allows reassignment in pattern when at least one name is new", () => {
    ...
    expect(I.evaluate(AST.identifier("x"))).toEqual({ kind: "i32", value: 3 });
    expect(I.evaluate(AST.identifier("z"))).toEqual({ kind: "i32", value: 4 });
  });
