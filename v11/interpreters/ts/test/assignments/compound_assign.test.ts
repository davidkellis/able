...
describe("v10 interpreter - compound assignments", () => {
  test("identifier targets for arithmetic and bitwise", () => {
    ...
    expect(I.evaluate(AST.identifier("x"))).toEqual({ kind: 'i32', value: 5 });
    I.evaluate(AST.assignmentExpression("<<=", AST.identifier("x"), AST.integerLiteral(1)));
    expect(I.evaluate(AST.identifier("x"))).toEqual({ kind: 'i32', value: 10 });
  });
  ...
  test("struct field and array index compound assignment", () => {
    ...
    expect(px).toEqual({ kind: 'i32', value: 5 });
    ...
    expect(a1).toEqual({ kind: 'i32', value: 8 });
  });
});
