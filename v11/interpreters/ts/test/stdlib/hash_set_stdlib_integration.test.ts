...
describe("stdlib-backed hash set helpers", () => {
  test("HashSet operations run via ModuleLoader", async () => {
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "able-hash-set-stdlib-"));
    try {
      await fs.writeFile(path.join(tmpRoot, "package.yml"), "name: hash_set_stdlib\n", "utf8");
      await fs.writeFile(
...
fn main() -> i32 {
  set := HashSet.new()
  if set.is_empty() { set.add(1) }
  set.add(2)
  set.add(2) ## duplicate insert should be ignored

  total := set.size()
  if set.contains(1) { total = total + 3 }
  if set.contains(2) { total = total + 4 }
  if set.remove(1) { total = total + 5 }
  if !set.contains(1) { total = total + 6 }
  if !set.remove(9) { total = total + 7 }
  total
}
`.trimStart(),
        "utf8",
      ...
      expect(readInteger(result)).toBe(27);
    } finally {
