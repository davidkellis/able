...
- **Canonical schema**: `PackageSummary` captures the public API for each
  ...
  `interpreter10/src/typechecker/diagnostics.ts` and mirrors the Go struct in
  `interpreter10-go/pkg/typechecker/program_checker.go`.
- **Go CLI**: `interpreter10-go/cmd/able/main.go` prints the summary block under
...
- **Bun CLI/harness**: the Bun fixture runner (`interpreter10/scripts/run-fixtures.ts`)
