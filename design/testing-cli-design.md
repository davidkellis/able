...
- Support snapshot workflows (`--update-snapshots`) and offer structured outputs suitable for automation (doc/progress/TAP/JSON).
...
Options:
  ...
  --update-snapshots        Enable snapshot update mode; mismatches overwrite snapshots
...
- Tag/name filters align with the framework metadata generated by RSpec-style suites.

## High-Level Flow

1. **Prepare Environment**
   - Parse CLI flags; validate conflicting options (e.g., `--list` with `--repeat` should warn but still run discovery).
   - Toggle snapshot update mode via `snapshot_set_update_mode(true)` when `--update-snapshots` is set; ensure it resets to `false` before exit.
   - (Future) configure default snapshot directory/location for file-backed stores.
...
3. **Discovery**
   ...
   - Call `able.testing.harness.discover_all(request)`.
...
5. **Reporter Selection**
   - Map `--format` to reporter:
     - `doc` → `DocReporter` writing to stdout.
     - `progress` → `ProgressReporter`, flush on finish.
...
7. **Execution**
   - Call `able.testing.harness.run_plan` with `TestPlan { descriptors }`, run options, reporter.
...
9. **Cleanup**
   - Reset snapshot update mode to `false`.
...
- Snapshot mode toggles and summary output.
- Exit code determination.

### Stdlib (Already Implemented)

- Framework registration and discovery/run logic per framework.
- Expectation matchers, snapshot store interface.
- Reporter implementations.
- Harness `discover_all`, `run_plan`, `run_all`.

## Snapshot Strategy (Stub)

- Current store is in-memory; CLI should eventually swap in file-backed store:
  - On CLI startup: create `SnapshotStore` that reads/writes `__snapshots__/<suite>.<ext>`.
  - Inject via a setter (`snapshot_set_store` future helper) or keep global store but swap implementation before module load.
  - Respect update mode for writing/overwriting files.
  - Provide diff output (likely using a formatter) in `details`.
- For now: CLI toggles update mode; tests rely on in-memory store. Document this limitation.
...
- Define file-backed snapshot store and CLI integration (follow-up item).
