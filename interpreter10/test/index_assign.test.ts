...
describe("v10 interpreter - index & member assignment", () => {
  test("array index mutation", () => {
    ...
    expect(I.evaluate(AST.identifier("a"))).toEqual({ kind: 'array', elements: [{ kind: 'i32', value: 1 }, { kind: 'i32', value: 9 }] });
  });
  ...
  test("named struct field mutation", () => {
    ...
    expect(I.evaluate(AST.memberAccessExpression(AST.identifier("p"), "x"))).toEqual({ kind: 'i32', value: 5 });
  });
});
